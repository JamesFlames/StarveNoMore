...
- [ ] **Add a lean root `CLAUDE.md`** that Claude Code auto-loads. Keep it under
...
- [ ] **Add a `TASKMAP.md`** (or a "Task → files" section in `CLAUDE.md`): a
      routing table mapping common jobs to the exact files to open. Draft rows:
  - [ ] "Change a card's stats/text" → `content/cards_*.csv` (+ regenerate step)
  - [ ] "Add/adjust a player action" → `lua/actions.lua` + `lua/ui_actionbar.lua`
  - [ ] "Change combat math" → `lua/combat.lua`
  - [ ] "Change a Dawn event" → `lua/effects/dawn_effects.lua` + `content/cards_*.csv`
  - [ ] "Change the day/night flow" → `lua/day_loop.lua`, `lua/night.lua`
  - [ ] "Change victory/defeat" → `lua/tick_victory.lua`
  - [ ] "Change in-game rules text" → `content/notebook/*.md` + regenerate
  - [ ] "Change a UI panel" → matching `lua/ui_*.lua` + `xml/*.xml`
  - [ ] "Rebalance" → `scripts/simulate_balance.py`, `content/cards_*.csv`
- [ ] **Confirm `SYMBOLS.md` is the first stop for "where is function X".**
      ...
      full-file reads.
- [ ] **Verify the auto-generated-file warnings are unmissable.** Each of the 6
...
- [ ] **Per-directory stubs** so scoped tasks load scoped context. Add a short

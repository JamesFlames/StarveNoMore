...
- [possible-design-improvements-to-snm.md](possible-design-improvements-to-snm.md) — the **game-design** review that produced the 2026-07 design pass: 17 rules/balance/onboarding findings against `Archive/PrinciplesOfGoodBoardGames.md` (esp. its Part IV co-op / attrition / accessibility chapters), each with evidence, a proposal, and how to verify it. **All 17 are now shipped** — the doc is kept as the rationale record; the rules themselves live in `docs/design/` and the change is logged in `CHANGELOG.md`. Read it for *why* a rule looks the way it does, never as the rule itself.
- [GoodForAiPlan.md](GoodForAiPlan.md) — the **AI-workflow** plan (2026-07), successor to the retired compartmentalise/structural programs. Where the *navigation* layer is now good but the remaining cost is elsewhere: session bootstrap (deps, permissions, hooks), the untested interactive surface (79% of XML click handlers and 59% of Lua globals are never named in a test), the `gameState` schema, and the size of this file plus `SYMBOLS.md`. Each item carries measured evidence, the change, and how to verify it; section 2 holds the commands that reproduce every number. Read it before starting a "make the repo better for agents" task — and check its scoreboard before re-measuring anything.
...
- `scripts/normalize_standee_art.py` — Derives `art/characters/<name>_standee.png`: cuts the backdrop to transparent (opt-in per character via `CUT_BACKGROUND`) and fits every figure to a common 512×1024. **`ASSET_MAP` points at the `_standee` files.** See "Derived art" below for why both exist.

...
- [possible-design-improvements-to-snm.md](possible-design-improvements-to-snm.md) — the **game-design** counterpart to `structuralimprovements.md`: 17 proposed rules/balance/onboarding changes, reviewed against `Archive/PrinciplesOfGoodBoardGames.md` (esp. its Part IV co-op / attrition / accessibility chapters), each with evidence, a proposal, and how to verify it. Proposals only — nothing here is implemented. A working doc, not a design doc: when an item ships, fold it into `docs/design/` + `CHANGELOG.md`.

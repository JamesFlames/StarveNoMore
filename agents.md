...
- **Fixed resource costs are auto-paid**: `verifyAndPayResources` (ui_actionbar_core.lua) checks the held count, deducts it, and blocks the action (with an action refund) if short. Used by Cleanse, Appease-Treeguard, Barricade, **and Craft** (`doCraft` now auto-pays `MARKET_COSTS`, plus a Scarcity surcharge at Doom ≥ 15 auto-picked from the most-held resource). Cooking's ingredients stay honor-system (no per-recipe ingredient data). There is no Discard Tray any more — nothing is dropped to pay.

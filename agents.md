...
- **`test_regression_guards.py`** — generic guards for the *classes* of bug that keep recurring in playtest, each encoding one hard-won lesson (see `docs/tts-runtime.md`): laid-out object groups (player boards, location tiles) must not overlap at spawn (physics scatters an overlapping stack); text-gadget objects (Notecard/Counter) must be upright (rotY≈0), not 180 (the upside-down Quick Start); button text must clear a minimum contrast against its own background (the dark-on-dark readability complaints); and **every `takeObject` callback that dereferences its object handle must wrap it in pcall/safecall** — a dead handle throws "cannot access field … of userdata", the crash hit three times this session (pry/dawn/threat) and the guard found five more latent ones. Runtime companions live in `test_lua_actions.py`: `TestResourceModelRobustness` (held resources stay correct when a board is dragged away — the gather-shows-zero class) and `TestStatDisplayNeverStale` (the left stat box shows live stats with no active player, not a frozen snapshot).

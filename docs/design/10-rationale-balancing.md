...
- **Doom track rate** — retuned 2026-07 to the §15.6 table (3p [1,1,1,1] / 4p [1,1,1,2] / 5p [1,1,2,2], no boss arrival Doom) after simulation showed the old table made 5p nearly unwinnable. The residual risk then inverted: with a gentle fixed clock, most Doom became responsive (festering). **Knob taken (batch 5, 2026-07): 4p Phase 4 rate +2 → +3**, the third of this section's ordered knobs, after the Last Nerve valve put the best simulated lines at 50–52% — above §20.2's band. It lands them at 40–42% with losses still ~100% on Days 6–7, and Nightmare at 13% (above its 8% floor). Phase 4 only: both phases overshoot to ~22%, Phase 3 alone undershoots at ~35–39%. Festering is still untouched, and remains the thing to leave alone.
...
- **Nightmare was unwinnable, and is now winnable — barely (retuned 2026-07).** A flat Doom +1 in every phase measured at **0–6%** for the best lines at 4 players. That is not a hard mode, it is a broken one, and the cause is compounding: most Doom is already responsive (§15.1), so a flat surcharge from Day 1 taxes the half of the week §14 wants calm and runs the clock out before the climax. **Knob taken: the surcharge moved to Phases 3–4 only, and the Source went to 9 HP** (one above Standard's calibrated 8) so the finale is a harder *fight* rather than the same fight on a shorter fuse. Best line ~21% with **100% of losses on Days 6–7** — a near-miss finish, which is what this list's calibration gate asks for. Two things to keep in mind: the probe *understates* Nightmare (it cannot model `minPhase`, so it never sees the Strange Days deck opening), and the next rungs in either direction are Source 8 (25%) and Source 10 (16.6%). The measured ladder is in §17.2.
- **Last Nerve (§10.1.1) — measured, and it moved the number.** Simulated at 400 games/policy, 4 players, comparing against the same build with the valve disabled:
  ...
  **This was deferred once, then compensated in batch 5.** The original position — per [§17](../../Archive/PrinciplesOfGoodBoardGames.md), Improving an Existing Game — was that Last Nerve, the Haunted buy-in, the secret-Dusk variant and the difficulty axis all push difficulty the same way and must not be evaluated together, so Standard's numbers should stay put pending table data. **Batch 5 took the third ordered knob anyway** (4p Phase 4 Doom +2 → +3, §15.6), on the simulator's evidence alone, restoring 40–42%. The §17 caution still stands and now applies to the *playtest*: four changes moved the same direction and one moved back, so a table result that differs from 40–50% cannot be attributed to any single one of them — it can only say whether the band is right. All three knobs were priced first (see [balance-simulation.md](../agents/balance-simulation.md)): knob 1, dropping the Rest half, is worth ~0 pp as predicted here; knob 2, Source HP 8 → 9, lands 45.8–47.8% and remains the unused gentler alternative if 40–42% proves too harsh.

...
**Provenance.** The day tracks and creature clips are Klei's (*Don't Starve*);
...
- **Day start (Dawn):** `Audio.startDayAmbience()` picks ONE suburban track
  and loops it until nightfall — a mid-day track
...
- Clips **shorter than `WATCH_DELAY` are never watched**: they have already
  ...
  load does. That is every Treeguard and Eye of Terror clip (all under 4s) and Deerclops'
  attack and death clips; only its distant calls run long enough to watch.

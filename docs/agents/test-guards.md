...
- **`test_lua_effect_flags.py`** — **every ongoing rule the game announces must be a rule something enforces.** The same bug class as reachability, one level down: not a function nothing calls but a *flag* nothing reads. A Dawn card sets `ongoingDawnEffects.moveCostPlus1`, chat announces it, `EFFECT_RULES` lists it in the Active Rules panel — and `doMove` has never heard of it, so the table plays around a rule that does not happen. A 2026-07 audit found eleven at once. A flag counts as enforced only when a non-`ui_` file reads it: printing a rule is what the panel is for, and printing it was exactly what all eleven were doing instead. `DISPLAY_ONLY` allowlists the genuinely panel-only flags with a reason each; a companion check catches the reverse — a flag a card sets that the panel never shows. Behaviour lives in `test_lua_ongoing_effects.py`.
- **`test_market_wiring.py`** — **every Market card is either wired or listed with a reason.** 49 craftable cards each print a mechanical effect; 33 were wired to nothing, and the mod gives players no way to apply one by hand (no Use Item verb outside `items.lua`, no stat editor, stats live in `gameState`). So the card was not "resolved by the table" — it did nothing at all, and looked exactly like a card that was deliberately social. This does not demand every card be scripted; it demands the split be written down. `UNWIRED` carries a reason per card, a companion check ejects one the moment the Lua reads it, and a third asserts `USE_ITEMS` still matches the numbers on the card faces.
